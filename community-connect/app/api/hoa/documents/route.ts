...
import { listHoaDocuments } from "@/lib/api/services/enterprise";
import { z } from "zod";
...
const docSchema = z.object({
  ...
  url: z.string().url(),
  category: z.enum(["BYLAWS", "MINUTES", "FINANCIAL", "NOTICE", "OTHER"]).default("OTHER"),
});
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const items = await withDbTimeout(listHoaDocuments(communityId));
    return jsonOk({ items });
  } catch (err) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const item = await withDbTimeout(
      prisma.hoaDocument.create({ data: { communityId, ...parsed.data } })
    );
