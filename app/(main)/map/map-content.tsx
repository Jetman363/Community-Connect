...
function layerFromQuery(value: string | null): string {
  if (value && VALID_LAYERS.has(value)) return value;
  return "all";
}

export function MapContent() {
  const searchParams = useSearchParams();
  const [layer, setLayer] = useState(() => layerFromQuery(searchParams.get("layer")));

  useEffect(() => {
    setLayer(layerFromQuery(searchParams.get("layer")));
  }, [searchParams]);
