...
class KafkaBackend(EventBackend):
    """Durable event routing through Apache Kafka with signed JSON envelopes."""
    ...
    async def publish(self, stream: str, message: dict[str, Any]) -> None:
        ...
        logger.debug(
            ...
            extra={"topic": stream, "partition": record.partition, "offset": record.offset},
        )
    ...
    async def read(self, stream: str, last_id: str, count: int) -> list[dict[str, Any]]:
        ...
        try:
            ...
            topic_partitions = sorted(TopicPartition(stream, partition) for partition in partitions)
            consumer.assign(topic_partitions)

            if last_id == "0":
                await consumer.seek_to_beginning(*topic_partitions)
            else:
                start_offset = self._resolve_start_offset(last_id)
            ...
            while len(collected) < count and idle_rounds < max_idle_rounds:
                batch = await consumer.getmany(
                    timeout_ms=settings.kafka_read_timeout_ms,
                    max_records=count - len(collected),
                )
                if not batch:
                    idle_rounds += 1
                    continue
                ...
                for records in batch.values():
                    for record in records:
                        collected.append(
                            {
                                "id": str(record.offset),
                                "data": self._parse_record(record.value),
                            }
                        )
    ...
    @staticmethod
    def _resolve_start_offset(last_id: str) -> int:
        try:
